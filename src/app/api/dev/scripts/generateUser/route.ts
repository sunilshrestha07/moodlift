...
function generateRandomUser() {
    ...
    const diagnoses = ["Depression", "Anxiety", "PTSD", "OCD", "Bipolar Disorder"];
...
export async function GET() {
    try {
        ...
        return NextResponse.json({ randomUserData, message: "user created" }, { status: 200 });
    } catch (error) {
        console.error("Error generating random user:", error);
        return NextResponse.json({ error: "Failed to generate random user" }, { status: 500 });
    }
}
